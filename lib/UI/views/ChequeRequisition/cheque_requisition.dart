...
import 'package:Expense/core/models/cheque_requisition.dart';
import 'package:Expense/core/models/cheque_requisition_unpaid.dart';
...
class ChequeRequisitionScreen extends StatefulWidget {
  @override
  _ChequeRequisitionScreenState createState() => _ChequeRequisitionScreenState();
}

class _ChequeRequisitionScreenState extends State<ChequeRequisitionScreen> {
  final _formKey = GlobalKey<FormState>();

  String pageAppTitle = 'Cheque Requisition';
  ...
  void _addNew() async {
    if (_formKey.currentState.validate()) {
      ...
      // Make API call
      var endPoint = '$userId/chequerequisitions';
      var result = await CallApi().postAuthData(data, endPoint);
      if (result.statusCode == 201) {
        if (this.mounted) {
          setState(() {
            fetchChequeRequisition();
            loading = false;
          });
        }
      } else {
  ...
  Widget build(BuildContext context) {
    // Add Cheque Requisition Form
    void _addForm() {
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text("Cheque Requisition Form"),
            content: loading
                ? Loading()
                : ListView(
                    children: <Widget>[
                            ...
                            children: <Widget>[
                              TextFormField(
                                decoration: InputDecoration(
                                  hintText: 'Payee',
                                  labelText: 'Cheque Requisition Payee',
...
class UnpaidRequest extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: FutureBuilder(
        future: fetchUnpaidChequeRequisition(),
...
class CashList extends StatelessWidget {
  final List<ChequeRequisitionData> cashData;
  ...
  _showPettyCashModalBottomSheet(context, ChequeRequisitionData cashData) {
                ...
                child: Column(
                  children: <Widget>[
                    ListTile(
                      ...
                      subtitle: dialogSubTitle(cashData.requisitionOn),
                    ),
                    ListTile(
                    ...
                    ListTile(
                      ...
                      subtitle: cashData.budgeted == null
                    ...
                    ListTile(
                      ...
                      subtitle: cashData.approved == "YES"
                          ? dialogSubTitle(
                    ...
                    ListTile(
                      ...
                      subtitle: cashData.approvalComment != null
                          ? dialogSubTitle(cashData.approvalComment)
                          : Text(''),
                    ),
                    ListTile(
...
class UnpaidCashData extends StatelessWidget {
  final List<UnpaidChequeRequisitionData> unpaidCashData;
  final egoFormata = new NumberFormat("#,##0.00", "en_NG");
  ...
  // Show Petty Cash Sheet
  _showPettyCashModalBottomSheet(
      context, UnpaidChequeRequisitionData unpaidCashData) {
    showModalBottomSheet(
      context: context,
      builder: (BuildContext context) {
        return Container(
          ...
          child: ListView(
            padding: const EdgeInsets.all(15),
            children: <Widget>[
              pageTitle(unpaidCashData.payee),
              Divider(),
              Container(
                child: Column(
                  children: <Widget>[
                    ListTile(
                      title: dialogTitle("Transaction Date:"),
                      subtitle: dialogSubTitle(unpaidCashData.requisitionOn),
                    ),
                    ListTile(
                      title: dialogTitle("Transaction Number:"),
                      subtitle:
                    ...
                    ListTile(
                      title: dialogTitle("ERMC Approval:"),
                      subtitle: unpaidCashData.budgeted == "YES"
                          ? dialogSubTitle("ERMC has approved your request.")
                          : dialogSubTitle(
                              "ERMC is yet to approve your request"),
                    ),
                    ListTile(
                      title: dialogTitle("Managing Director's Approval:"),
                      subtitle: unpaidCashData.approved == "YES"
                          ? dialogSubTitle(
                              "Managing director's has approved your request.")
                          : dialogSubTitle(
                              "Managing director's is yet to approve your request"),
                    ...
                    ListTile(
                        title: dialogTitle("Managing Director's Comment:"),
                        subtitle: unpaidCashData.approvalComment != null
                            ? dialogSubTitle(unpaidCashData.approvalComment)
                            : Text('')),
                    ListTile(
                      title: dialogTitle("Payment Status:"),
                    ...
                    ListTile(
                      ...
                      subtitle: dialogSubTitle(unpaidCashData.description),
                    ),
                    ListTile(
                      title: dialogTitle("Amount Requested:"),
                      subtitle: dialogSubTitleAmount(
          ...
          child: Padding(
              ...
              child: unpaidData != null
                  ? ListTile(
                      ...
                      title: Text(unpaidData.payee),
                      subtitle: Text(
                        truncate(unpaidData.description, 50,
    ...
    if (result.statusCode == 201) {
      if (this.mounted) {
        setState(() {
          ...
          isError = false;
          success =
              "Cheque Requisition request successfully concluded. This transacton will now be marked as completed.";
        });
      }
    } else {
      if (this.mounted) {
