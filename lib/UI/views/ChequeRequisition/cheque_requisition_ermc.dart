...
import 'package:Expense/core/models/cheque_requisition_ermc.dart';
...
class ChequeRequisitionErmcScreen extends StatefulWidget {
  @override
  _ChequeRequisitionErmcScreenState createState() =>
      _ChequeRequisitionErmcScreenState();
}

class _ChequeRequisitionErmcScreenState
    extends State<ChequeRequisitionErmcScreen> {
  ...
  _showPettyCashModalBottomSheet(
      context, ChequeRequisitionErmcData data) {
        ...
        children: <Widget>[
          ...
          Container(
            ...
            child: Padding(
              ...
              child: FutureBuilder(
                future: fetchChequeRequisitionErmcData(),
...
class _ApprovalPageState extends State<ApprovalPage> {
  ...
  void _approveStaffRequest(int id) async {
    if (_formKey.currentState.validate()) {
      ...
      var data = {
        'ucr_id': id,
        'budgeted': approvedAmount,
        'checked_by': userId,
      };

      // Make API call
      var endPoint = 'chequerequisitions/user/$userId/ermc';
      var result = await CallApi().putAuthData(data, endPoint);
      if (result.statusCode == 201) {
        if (this.mounted) {
          setState(() {
            fetchChequeRequisitionErmcData();
            loading = false;
            Navigator.of(context).pop();
          });
        }
  ...
  @override
  Widget build(BuildContext context) {
    final ChequeRequisitionErmcData pcData = ModalRoute.of(context).settings.arguments;
    return Scaffold(
      appBar: AppBar(
