...
import 'package:Expense/core/models/cheque_requisition_hod.dart';
...
class ChequeRequisitionHodScreen extends StatefulWidget {
  @override
  _ChequeRequisitionHodScreenState createState() =>
      _ChequeRequisitionHodScreenState();
}

class _ChequeRequisitionHodScreenState
    extends State<ChequeRequisitionHodScreen> {
  ...
  _showPettyCashModalBottomSheet(
      context, ChequeRequisitionHodData data) {
        ...
        children: <Widget>[
          ...
          Container(
            ...
            child: Padding(
              ...
              child: FutureBuilder(
                future: fetchChequeRequisitionHodData(),
...
class _ApprovalPageState extends State<ApprovalPage> {
  ...
  void _approveStaffRequest(int id) async {
    if (_formKey.currentState.validate()) {
      ...
      // Make API call
      var endPoint = 'chequerequisitions/user/$userId/hod';
      var result = await CallApi().putAuthData(data, endPoint);
      if (result.statusCode == 201) {
        if (this.mounted) {
          setState(() {
            fetchChequeRequisitionHodData();
            loading = false;
            Navigator.of(context).pop();
          });
        }
  ...
  @override
  Widget build(BuildContext context) {
    final ChequeRequisitionHodData pcData = ModalRoute.of(context).settings.arguments;
    return Scaffold(
      appBar: AppBar(
