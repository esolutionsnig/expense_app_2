...
class CashAdvanceDataErmc {
  ...
  // Static Method
  factory CashAdvanceDataErmc.fromJson(Map<String, dynamic> json) {
    return CashAdvanceDataErmc(
      id: json["id"],
      userId: json["user_id"],
      title: json["title"],
...
// Fetch data from API
Future<List<CashAdvanceDataErmc>> fetchCashAdvanceErmc() async {
  ...
  var user = userData[0];
  var userId = user["id"];
  var endPoint = 'cashadvances/user/$userId/approvals/ermc';
  ...
  try {
    ...
    if (result.statusCode == 200) {
      ...
      final listOfCashAdvance = await pcs.map<CashAdvanceDataErmc>((json) {
        return CashAdvanceDataErmc.fromJson(json);
      }).toList();
