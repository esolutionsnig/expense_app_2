...
class CashAdvanceDataHod {
  ...
  // Static Method
  factory CashAdvanceDataHod.fromJson(Map<String, dynamic> json) {
    return CashAdvanceDataHod(
      id: json["id"],
      userId: json["user_id"],
      title: json["title"],
...
// Fetch data from API
Future<List<CashAdvanceDataHod>> fetchCashAdvanceHod() async {
  ...
  var user = userData[0];
  var userId = user["id"];
  var endPoint = 'cashadvances/user/$userId/approvals/hod';
  ...
  try {
    ...
    if (result.statusCode == 200) {
      ...
      final listOfCashAdvance = await pcs.map<CashAdvanceDataHod>((json) {
        return CashAdvanceDataHod.fromJson(json);
      }).toList();
