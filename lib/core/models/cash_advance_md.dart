...
class CashAdvanceDataMd {
  ...
  // Static Method
  factory CashAdvanceDataMd.fromJson(Map<String, dynamic> json) {
    return CashAdvanceDataMd(
      id: json["id"],
      userId: json["user_id"],
      title: json["title"],
...
// Fetch data from API
Future<List<CashAdvanceDataMd>> fetchCashAdvanceMd() async {
  ...
  var user = userData[0];
  var userId = user["id"];
  var endPoint = 'cashadvances/user/$userId/approvals/md';
  ...
  try {
    ...
    if (result.statusCode == 200) {
      ...
      final listOfCashAdvance = await pcs.map<CashAdvanceDataMd>((json) {
        return CashAdvanceDataMd.fromJson(json);
      }).toList();
