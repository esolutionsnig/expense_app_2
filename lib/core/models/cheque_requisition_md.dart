...
class ChequeRequisitionMdData {
  ...
  // Statis Method
  factory ChequeRequisitionMdData.fromJson(Map<String, dynamic> json) {
    return ChequeRequisitionMdData(
      id: json["id"],
      userId: json['user_id'],
      payee: json['payee'],
      amount: json['amount'],
...
// Fetch data from API
Future<List<ChequeRequisitionMdData>> fetchChequeRequisitionMdData() async {
  ...
  var user = userData[0];
  var userId = user["id"];
  var endPoint = 'chequerequisitions/user/$userId/approvals/md';
  ...
  try {
    ...
    if (result.statusCode == 200) {
      ...
      final listOfChequeRequisition = await pcs.map<ChequeRequisitionMdData>((json) {
        return ChequeRequisitionMdData.fromJson(json);
      }).toList();
