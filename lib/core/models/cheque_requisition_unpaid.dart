...
class UnpaidChequeRequisitionData {
  int id;
  int userId;
  String payee;
  int amount;
  ...
  // Statis Method
  factory UnpaidChequeRequisitionData.fromJson(Map<String, dynamic> json) {
    return UnpaidChequeRequisitionData(
      id: json["id"],
      userId: json['user_id'],
      payee: json['payee'],
      amount: json['amount'],
...
// Fetch data from API
Future<List<UnpaidChequeRequisitionData>> fetchUnpaidChequeRequisition() async {
  ...
  // var endPoint = 'chequerequisitions/user/$userId/approvals/hod';
  ...
  try {
    ...
    if (result.statusCode == 200) {
      ...
      final pcs = mapResult["unpaidData"].cast<Map<String, dynamic>>();
      // Get list of petty cash
      final listOfChequeRequisition = await pcs.map<UnpaidChequeRequisitionData>((json) {
        return UnpaidChequeRequisitionData.fromJson(json);
      }).toList();
